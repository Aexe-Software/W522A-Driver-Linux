...
/* WiFi-BT exclusive radio mutex — implemented in aml_sdio (w1_sdio.c).
 * hci_uart.ko links against aml_sdio.ko via KBUILD_EXTRA_SYMBOLS. */
...
static int aml_parse_dt(struct aml_serdev *amldev)
{
	...
	amldev->bt_en_gpio = devm_gpiod_get_optional(pdev, "bt-enable",
					GPIOD_OUT_LOW);
	if (IS_ERR(amldev->bt_en_gpio)) {
		if (PTR_ERR(amldev->bt_en_gpio) == -EBUSY) {
			dev_warn(pdev, "bt-enable GPIO busy, assuming power is already managed");
			amldev->bt_en_gpio = NULL;
		} else {
...
static int aml_power_on(struct aml_serdev *amldev)
{
	...
	if (!IS_ERR_OR_NULL(amldev->bt_supply)) {
	...
	if (!IS_ERR_OR_NULL(amldev->lpo_clk)) {
	...
	if (!IS_ERR_OR_NULL(amldev->bt_en_gpio))
...
static int aml_open(struct hci_uart *hu)
{
	...
	/* WiFi-BT exclusive radio mutex: BT and 2.4 GHz WiFi on W155S2 share
	 * the same RF front-end and cannot run simultaneously. Claim the radio
	 * before powering on the BT subsystem; fail if WiFi is already active.
	 * The user must bring the WiFi interface DOWN first. */
	if (w1_wifi_bt_try_claim(W522A_RADIO_BT) != 0) {
		pr_warn("W522A-BT: Bluetooth blocked — WiFi radio is active. "
			"Run: ip link set w522a down\n");
		return -EBUSY;
	}

	err = aml_parse_dt(amldev);
	if (err) {
		w1_wifi_bt_release(W522A_RADIO_BT);
		return err;
	}
...
static int aml_close(struct hci_uart *hu)
{
	...
	/* Do NOT call serdev_device_close() here. The hci_serdev framework owns
	 * the serdev port lifecycle: hci_uart_register_device() opens it and
	 * hci_uart_unregister_device() closes it. Closing it again from the proto
	 * .close op double-closes the underlying tty -> refcount underflow /
	 * use-after-free (tty_kref_put) and a WARNING/oops on rmmod. */
...
static int aml_setup(struct hci_uart *hu)
{
	...
	if (amldev->serdev_hu.proto->oper_speed !=
	    amldev->serdev_hu.proto->init_speed) {
