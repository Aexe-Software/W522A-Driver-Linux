...
{
    unsigned long _scratch;
    if (!WIFI_MAC_LIKELY_KERNEL_PTR(p))
        return 0;
    if (copy_from_kernel_nofault(&_scratch, p, sizeof(_scratch)) != 0)
        return 0;
    return 1;
}
...
#define VLSI_VALID_VIFMAC(vif)                                        \
    (WIFI_MAC_LIKELY_KERNEL_PTR(vif) &&                               \
     WIFI_MAC_LIKELY_KERNEL_PTR((vif)->vm_wmac) &&                    \
     WIFI_MAC_LIKELY_KERNEL_PTR((vif)->vm_wmac->drv_priv))

#define VLSI_VALID_DRV_OP(wmac, op)                                   \
    (WIFI_MAC_LIKELY_KERNEL_PTR(wmac) &&                              \
     WIFI_MAC_LIKELY_KERNEL_PTR((wmac)->drv_priv) &&                  \
     WIFI_MAC_LIKELY_KERNEL_PTR((wmac)->drv_priv->drv_ops.op))

#ifndef VLSI_LIST_MAX_ITERS
    ...
    for (struct { struct list_head *h; struct list_head *c; int i; }  \
            _vlsi_it = { (head),                                      \
                         vlsi_kp_alive(head)                          \
                            ? (head)->next : (head),                  \
                         0 };                                         \
         _vlsi_it.i < VLSI_LIST_MAX_ITERS                             \
            && _vlsi_it.c != _vlsi_it.h                               \
            && vlsi_kp_alive(_vlsi_it.c)                              \
            && vlsi_kp_alive(_vlsi_it.c->next)                        \
            && ((pos) = list_entry(_vlsi_it.c, typeof(*(pos)),        \
                                   member),                           \
                (n)   = list_entry(_vlsi_it.c->next, typeof(*(n)),    \
                                   member),                           \
                WIFI_MAC_LIKELY_KERNEL_PTR(pos));                     \
         _vlsi_it.c = &(n)->member, _vlsi_it.i++)

static __inline void
wifi_mac_KeyUpdateBegin(struct wlan_net_vif *wnet_vif)
{
    void (*fn)(struct wlan_net_vif *);

    if (wnet_vif == NULL)
        return;
    fn = wnet_vif->vif_ops.vm_key_update_begin;
    if (!WIFI_MAC_LIKELY_KERNEL_PTR(fn))
        return;
    fn(wnet_vif);
}
...
wifi_mac_KeyUpdateEnd(struct wlan_net_vif *wnet_vif)
{
    ...
    if (wnet_vif == NULL)
        return;
    fn = wnet_vif->vif_ops.vm_key_update_end;
    if (!WIFI_MAC_LIKELY_KERNEL_PTR(fn))
        return;
    fn(wnet_vif);
}
...
wifi_mac_SetTim(struct wifi_station *sta, int set)
{
    ...
    void (*fn)(struct wifi_station *, int);

    if (sta == NULL || sta->sta_wnet_vif == NULL)
        return;
    wnet_vif = sta->sta_wnet_vif;
    fn = wnet_vif->vif_ops.vm_set_tim;
    if (!WIFI_MAC_LIKELY_KERNEL_PTR(fn))
        return;
    fn(sta, set);
}
...
static __inline  int wifi_mac_hdrsize(const void *data)
{
    ...
    if ((wh->i_fc[0]&WIFINET_FC0_TYPE_MASK) == WIFINET_FC0_TYPE_CTL)
        return size;
...
wifi_mac_is_ht_enable(struct wifi_station *sta)
{
    ...
    if ((sta->sta_ucastkey.wk_cipher == &wifi_mac_cipher_none)
        ...
        if (k && k->wk_cipher != NULL && k->wk_cipher->wm_cipher == WIFINET_CIPHER_WEP) {
            return 0;
        }
    }
