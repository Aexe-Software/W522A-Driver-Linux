...
{
    /* v15s: widened both fields from `unsigned short` to `unsigned int`.
     * VHT peers legitimately advertise A-MPDU exp = 7 -> 1 MiB - 1 =
     * 0xFFFFF which does not fit in 16 bits. See wifi_mac_sta.h. */
...
struct driver_ops
{
    ...
    /* v15s: widened maxampdu from `unsigned short` to `unsigned int`;
     * see wifi_mac_sta.h sta_maxampdu for rationale. */
