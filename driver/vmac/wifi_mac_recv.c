...
static void __wifi_mac_deliver_data(struct wifi_station *, struct sk_buff *,
                                    struct list_head *batch_list);
static void wifi_mac_flush_rx_batch(struct list_head *batch_list);
...
int wifi_mac_rx_get_wnet_vifid(struct wifi_mac *wifimac,struct wifi_frame *wh)
{
...

    {
        ...
        VLSI_FOR_EACH_ENTRY_SAFE(tmpwnet_vif, _vlsi_n,
                                 &wifimac->wm_wnet_vifs, vm_next)
        {
            struct wifi_station *mainsta = READ_ONCE(tmpwnet_vif->vm_mainsta);
            if (mainsta == NULL)
            {
                continue;
            }
            if (WIFINET_ADDR_EQ(mainsta->sta_bssid, bssid))
            {
...
int wifi_mac_input(void *station, struct sk_buff *skb, struct wifi_mac_rx_status *rs)
{
       ...
       )
    {
        switch (wnet_vif->vm_opmode)
        {
            case WIFINET_M_STA:
                ...
                if ((READ_ONCE(wnet_vif->vm_state) == WIFINET_S_CONNECTED) && (!WIFINET_IS_PROBERSP(wh))) {
                    if (atomic_read(&wnet_vif->vm_bmiss_count)) {
    ...
    switch (type)
    {
        ...
        case WIFINET_FC0_TYPE_MGT:
            ...
            if ((READ_ONCE(wnet_vif->vm_state) == WIFINET_S_CONNECTED) && (sta->sta_flags_ext & WIFINET_NODE_MFP)) {
                if (!(wh->i_fc[1] & WIFINET_FC1_WEP)) {
...
int wifi_mac_input_all(struct wifi_mac *wifimac, struct sk_buff *skb, struct wifi_mac_rx_status *rs)
{
    ...
    VLSI_FOR_EACH_ENTRY_SAFE(wnet_vif, wnet_vif_next, &wifimac->wm_wnet_vifs, vm_next) {
        ...
        if (multicast) {
            if (((wnet_vif->vm_opmode == WIFINET_M_STA) || (wnet_vif->vm_opmode == WIFINET_M_P2P_CLIENT))
                && (READ_ONCE(wnet_vif->vm_state) == WIFINET_S_CONNECTED) && !(wifimac->wm_flags & WIFINET_F_SCAN)
...
wifi_mac_defrag(struct wifi_station *sta, struct sk_buff *skb, int hdrlen)
{
    ...
    else
    {
        if (sta->sta_rxfrag[0])     
        {
            ...
            if (os_skb_get_pktlen(skb) < hdrlen) {
                os_skb_free(skb);
                return NULL;
            }
...
static void wifi_mac_tcp_pkt_retransmit(struct wifi_station *sta, struct tcphdr *th)
{
    ...
    if (sta == NULL || sta->sta_wmac == NULL || sta->sta_wmac->drv_priv == NULL || th == NULL)
        return;
    ...
    for (i=0; i<HAL_NUM_TX_QUEUES; i++)
    {
        ...
        while (1)
        {
            ...
            if (ptxdesc == NULL || ptxdesc->txinfo == NULL || ptxdesc->txinfo->ptxdesc == NULL) {
                break;
            }
...
static void wifi_mac_recv_pkt_parse(struct wifi_station *sta, struct sk_buff *skb) {
    ...
    if (sta == NULL || skb == NULL || os_skb_data(skb) == NULL)
        return;
    ...
    if (eh->ether_type == __constant_htons(ETHERTYPE_PAE)) {
        if (pktlen < sizeof(struct ether_header) + 7)
            return;
        ...
        if (pktlen < sizeof(struct ether_header) + sizeof(struct iphdr))
            return;

        iphdrp = (struct iphdr *)((unsigned char *)eh + sizeof(struct ether_header));
        ip_hdr_len = iphdrp->ihl << 2;
        if (ip_hdr_len < sizeof(struct iphdr))
            return;

        l4_offset = sizeof(struct ether_header) + ip_hdr_len;
        if (pktlen < l4_offset)
            return;

        if (iphdrp->protocol == IPPROTO_TCP) {
            if (pktlen < l4_offset + sizeof(struct tcphdr))
                return;
            th = (struct tcphdr *)((unsigned char *)os_skb_data(skb) + l4_offset);
            tcp_hdr_len = th->doff << 2;
            if (tcp_hdr_len < sizeof(struct tcphdr) || pktlen < l4_offset + tcp_hdr_len)
                return;
            ...
            tcp_rx_payload_total += pktlen - (l4_offset + tcp_hdr_len);
         ...
         if (((uh->source == 0x4400) && (uh->dest == 0x4300))
                ...
                offset = 240;
                offset_max = pktlen - (l4_offset + sizeof(struct udphdr));

                while ((offset < offset_max) && (dhcp_p[offset] != 255))
                {
                    if (dhcp_p[offset] == 53) {
                    ...
                    next_offset = offset + dhcp_p[offset + 1] + 2;
                    if (next_offset <= offset || next_offset > offset_max)
                        break;
                    offset = next_offset;
                }
            } else {
    ...
    if (skb != NULL)
    {
        ...
        else
        {
            ...
            if (batch_list != NULL) {
                ...
                LIST_HEAD(tmp_list);
                list_add_tail(&skb->list, &tmp_list);
                local_bh_disable();
                netif_receive_skb_list(&tmp_list);
                local_bh_enable();
            }
...
{
    if (batch_list == NULL || list_empty(batch_list))
        return;
    local_bh_disable();
    netif_receive_skb_list(batch_list);
    local_bh_enable();
}
                  ...
                  int rssi, unsigned int rstamp, unsigned short seq, unsigned short status)
{
    ...
    {
        case WIFINET_M_IBSS:
            if (READ_ONCE(wnet_vif->vm_state) != WIFINET_S_CONNECTED || seq != WIFINET_AUTH_OPEN_REQUEST)
            {
                wnet_vif->vif_sts.sts_rx_auth_mismatch++;
                return;
            }
        ...
        case WIFINET_M_STA:
            if ((READ_ONCE(wnet_vif->vm_state) != WIFINET_S_AUTH) || (seq != WIFINET_AUTH_OPEN_RESPONSE)) {
                wnet_vif->vif_sts.sts_rx_auth_mismatch++;
                    ...
                    unsigned short seq, unsigned short status)
{
    ...
    switch (wnet_vif->vm_opmode)
    {
        ...
        case WIFINET_M_HOSTAP:
            ...
            {
                WIFINET_DPRINTF_MACADDR( AML_DEBUG_CONNECT,
                    sta->sta_macaddr, "shared key auth bad state %u", READ_ONCE(wnet_vif->vm_state));
                estatus = WIFINET_STATUS_ALG;
                goto bad;
            }
            switch (seq)
                         ...
                         struct wifi_mac_scan_param* sp, struct wifi_station *sta)
{
    ...
    if (csa_cnt > 0)
    {
        ...
        os_timer_ex_cancel(&sta->csa_timer, CANCEL_NO_SLEEP);
    ...
    struct wifi_station *sta, struct sk_buff *skb, int rssi, unsigned int channel)
{
    ...
    if ((wnet_vif->vm_opmode == WIFINET_M_STA) && (sta->sta_associd != 0)
        ...
        if ((READ_ONCE(wnet_vif->vm_state) == WIFINET_S_CONNECTED) && (sta == wnet_vif->vm_mainsta)) {
            if (scan.tim != NULL) {
                struct wifi_mac_tim_ie *tim = (struct wifi_mac_tim_ie *) scan.tim;
    ...
    if ((wnet_vif->vm_opmode == WIFINET_M_IBSS)
        && (READ_ONCE(wnet_vif->vm_state) == WIFINET_S_CONNECTED) && WIFINET_ADDR_EQ(wh->i_addr3, wnet_vif->vm_mainsta->sta_bssid)) {
        ...
        if (wpa != NULL) {
            ...
            if (reason != 0) {
                ...
                printk_ratelimited(KERN_WARNING
                    ...
                    wnet_vif->wnet_vif_id, wh->i_addr2, reassoc,
                    (wpa[0] != WIFINET_ELEMID_RSN) ? "WPA" : "RSN", reason,
                    sta->sta_rsn.rsn_mcastcipher, sta->sta_rsn.rsn_mcastkeylen,
                    sta->sta_rsn.rsn_ucastcipher, sta->sta_rsn.rsn_ucastkeylen,
                    sta->sta_rsn.rsn_keymgmtset, sta->sta_rsn.rsn_caps,
                    sta->sta_authmode, wnet_vif->vm_flags,
                    wnet_vif->vm_mainsta->sta_flags_ext);
    ...
    struct wifi_station *sta, struct sk_buff *skb, unsigned int channel)
{
    ...
    if ((wnet_vif->vm_opmode != WIFINET_M_STA) || (READ_ONCE(wnet_vif->vm_state) != WIFINET_S_ASSOC)) {
        wnet_vif->vif_sts.sts_mng_discard++;
        return;
    }
    ...
    if (READ_ONCE(wnet_vif->vm_state) == WIFINET_S_ASSOC) {
    ...
    struct wifi_station *sta, struct sk_buff *skb,unsigned int channel)
{
    ...
    {
        if ((READ_ONCE(wnet_vif->vm_state) != WIFINET_S_CONNECTED) && (READ_ONCE(wnet_vif->vm_state) != WIFINET_S_ASSOC)
            && (READ_ONCE(wnet_vif->vm_state) != WIFINET_S_AUTH)) {
            wnet_vif->vif_sts.sts_mng_discard++;
            return;
        }

        if ((sta->sta_flags_ext & WIFINET_NODE_MFP) && (READ_ONCE(wnet_vif->vm_state) == WIFINET_S_ASSOC)) {
        ...
        switch (ia->ia_category) {
            case AML_CATEGORY_QOS:
                ...
                WIFINET_DPRINTF_STA(AML_DEBUG_ACTION, sta, "%s: QoS action mgt frames not supported, vm_state %d \n",
                    __func__, READ_ONCE(wnet_vif->vm_state));
                wnet_vif->vif_sts.sts_mng_discard++;
                break;
            ...
            case AML_CATEGORY_BACK:
                switch (ia->ia_action) {
                    ...
                    case WIFINET_ACTION_BA_ADDBA_RESPONSE:
                    {
                        ...
                        DPRINTF(AML_DEBUG_ADDBA, "%s:%d vm_state %d \n",__func__,__LINE__, READ_ONCE(wnet_vif->vm_state));
                        ...
                        *(unsigned short *)&baparamset = le16toh(*(unsigned short *)&addbaresponse->rs_baparamset);
                        ...
                        if (atomic_read(&tid->addba_exchangeinprogress) == 0) {
                        ...
                        DPRINTF(AML_DEBUG_ADDBA, "%s: ADDBA response . TID %d, buffer size %d,tid %d statuscode %d vm_state %d\n",
                            __func__, baparamset.tid, baparamset.buffersize,addbaresponse->rs_baparamset.tid,statuscode, READ_ONCE(wnet_vif->vm_state));
                    ...
                    case WIFINET_ACTION_BA_DELBA:
                        ...
                        *(unsigned short *)&delbaparamset= le16toh(*(unsigned short *)&delba->dl_delbaparamset);
                        ...
                        DPRINTF(AML_DEBUG_ADDBA, "%s: DELBA  . TID %d, initiator %d, reason code %d vm_state %d\n",
                            __func__, delbaparamset.tid, delbaparamset.initiator, reasoncode, READ_ONCE(wnet_vif->vm_state));
                        break;
...
wifi_mac_amsdu_subframe_decap(struct sk_buff *skb)
{
    ...
    if (unlikely(!llc)) {
        pr_warn_ratelimited("wifi_mac_recv: skb too short for ether_header\n");
        return;
    }
    eh_src.ether_type = llc->llc_un.type_snap.ether_type;
    if (unlikely(!os_skb_pull(skb, LLC_SNAPFRAMELEN))) {
        pr_warn_ratelimited("wifi_mac_recv: skb too short for LLC SNAP\n");
        return;
    }
...
wifi_mac_amsdu_input(struct wifi_station *sta, struct sk_buff *skb)
{
    ...
    if (os_skb_get_pktlen(skb) < sizeof(struct ether_header)) {
        pr_warn("AMSDU: skb too short for ether_header (%u < %zu)\n",
