...
#define HAL_RATECTRL_CW40_FLAG 0x0002    
#define HAL_RATECTRL_SGI_FLAG 0x0004    
#define HAL_RATECTRL_TX_SEND_SUCCESS 0X800
#define HAL_RATECTRL_USE_SAMPLE_RATE 0X1000

#define HAL_BEACON_PERIOD 0x0001ffff  
#define WLAN_CTRL_FRAME_SIZE 14   
...
struct hal_key_val
{
    unsigned char    kv_type;        
    unsigned char    kv_pad;
    unsigned short   kv_len;         
    unsigned char    kv_val[16];     
    unsigned char    kv_mic[8];      
    unsigned char    kv_txmic[8];    
} ;
    ...
    struct hal_key_val *_pk, unsigned char *_mac, unsigned short staaid)
{
    ...
    if (_pk->kv_type == HAL_KEY_TYPE_WEP)        
    {
        if (_pk->kv_len==13)
        {
