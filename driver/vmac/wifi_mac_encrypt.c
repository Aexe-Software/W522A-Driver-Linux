...
int wifi_mac_security_req(struct wlan_net_vif *wnet_vif, int cipher, int flags, struct wifi_mac_key *key)
{
    ...
    WIFINET_DPRINTF(AML_DEBUG_DEBUG," k->wk_keyix = %d new %s ,old %s\n", key->wk_keyix, cip->wm_name,
        (key->wk_cipher && key->wk_cipher != &wifi_mac_cipher_none) ? key->wk_cipher->wm_name : "none");
    oflags = key->wk_flags;
    ...
    if (key->wk_cipher != cip || key->wk_flags != flags) {
        ...
        {
            const struct wifi_mac_security *old_cip = READ_ONCE(key->wk_cipher);
            if (old_cip != NULL && old_cip != &wifi_mac_cipher_none && old_cip->wm_detach != NULL)
                old_cip->wm_detach(key);
        }
        key->wk_cipher = cip;
...
static int wifi_mac_security_delkey(struct wlan_net_vif *wnet_vif, struct wifi_mac_key *key, struct wifi_station *sta)
{
    ...
    const struct wifi_mac_security *cipher;
...

    {
        ...
        if (old == 0)
            return 0;
        if ((unsigned long)cmpxchg((unsigned long *)&key->wk_cipher,
                                   old, 0UL) != old)
            return 0;
    ...
    if (cipher != &wifi_mac_cipher_none &&
        ...
        cipher != &wifi_mac_cipher_ccmp &&
        cipher != &wifi_mac_cipher_wpi) {
        pr_warn_once("%s: dropped suspect cipher %p on key %p\n",
                     __func__, cipher, key);
        return 0;
    }
    ...
    if (key_index != WIFINET_KEYIX_NONE) {
        if (key_index < WIFINET_WEP_NKID) {
            ...
            DPRINTF(AML_DEBUG_KEY,
                    "%s: skipping wifi_mac_key_delete OOB keyix=%u\n",
                    __func__, key_index);
        }
    }
    ...
    const unsigned char macaddr[WIFINET_ADDR_LEN], struct wifi_station *sta)
{
    ...
    if (wnet_vif == NULL || key == NULL || key->wk_cipher == NULL)
        return 0;
...
wifi_mac_security_decap(struct wifi_station *sta, struct sk_buff *skb, int hdrlen)
{
    ...
    else
    {
        ...
        if (os_skb_get_pktlen(skb) < (sizeof(struct wifi_frame) + cip->wm_miclen) )
        {
            WIFINET_DPRINTF_STA( AML_DEBUG_ANY, sta,
                                 "%s: WEP data frame too short, len %u",
                                 __func__, os_skb_get_pktlen(skb));
...
tkip_enmic(struct wifi_mac_key *k, struct sk_buff *skb0, int force)
{
    ...
    if (force || (k->wk_flags & WIFINET_KEY_SWMIC))
    {
        ...
        const struct wifi_mac_security *cip_local = READ_ONCE(k->wk_cipher);
        if (cip_local == NULL || cip_local == &wifi_mac_cipher_none)
            return 0;
        ...
        if (skb_tailroom(skb) < cip_local->wm_miclen)
        {
        ...
        memcpy(os_skb_put(skb, cip_local->wm_miclen), mic, cip_local->wm_miclen);
    }
    return 1;
}
...
tkip_demic(struct wifi_mac_key *k, struct sk_buff *skb0, int hdrlen, int force)
{
    ...
    if ((k->wk_flags & WIFINET_KEY_SWMIC) && force)
    {
        ...
        michael_mic(k->wk_rxmic,
                    skb0, hdrlen, pktlen - (hdrlen + cip_local->wm_miclen),
                    mic);
        memcpy(mic0, os_skb_data(skb) + os_skb_get_pktlen(skb) - cip_local->wm_miclen,
               cip_local->wm_miclen);
        if (memcmp(mic, mic0, cip_local->wm_miclen))
        {
            wifi_mac_notify_mic_fail(wnet_vif, wh, k->wk_keyix);
            WIFINET_DPRINTF( AML_DEBUG_KEY,"%s  mic fail drop\n",ether_sprintf(wh->i_addr2));
            return 0;
        }
        os_skb_trim(skb, os_skb_get_pktlen(skb) - cip_local->wm_miclen);
    }
    ...
    unsigned char mic[WIFINET_WEP_MICLEN])
{
    ...
    for (;;)
    {
        ...
        if (space != 0)
        {
            ...
            if (os_skb_get_pktlen(skb) < sizeof(uint32_t) - space)
                break;
