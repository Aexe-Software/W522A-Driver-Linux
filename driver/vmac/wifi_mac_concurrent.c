                                            ...
                                            SYS_TYPE param2,SYS_TYPE param3, SYS_TYPE param4,SYS_TYPE param5)
{
    ...
    if (main_vmac == NULL || p2p_vmac == NULL || p2p_vmac->vm_p2p == NULL)
        return;
    ...
    SYS_TYPE param2,SYS_TYPE param3, SYS_TYPE param4,SYS_TYPE param5)
{
    ...
    if (atomic_read(&wifimac->wm_nrunning) > 1) {
        if (wifimac->wm_vsdb_flags & CONCURRENT_CHANNEL_SWITCH) {
            ...
            /* Pause TX before switching RF channel to avoid sending
             * frames on the wrong channel during the transition. */
            ...
            /* Resume TX after channel switch is complete. */
                                            ...
                                            SYS_TYPE param2,SYS_TYPE param3, SYS_TYPE param4,SYS_TYPE param5)
{
    ...
    if ((atomic_read(&wifimac->wm_nrunning) > 1) && (wifimac->wm_vsdb_flags & CONCURRENT_CHANNEL_SWITCH)) {
        if (wifimac->wm_vsdb_slot == CONCURRENT_SLOT_STA) {
    ...
    } else if ((atomic_read(&wifimac->wm_nrunning) < 2) && (wifimac->wm_vsdb_flags & CONCURRENT_CHANNEL_SWITCH)) {
        wifimac->wm_vsdb_flags = 0;
    }
}
...
void concurrent_vsdb_prepare_change_channel(struct wifi_mac *wifimac)
{
    ...
    if ((atomic_read(&wifimac->wm_nrunning) > 1 && wifimac->wm_vsdb_slot == CONCURRENT_SLOT_P2P && p2p_vmac->vm_p2p->p2p_role == NET80211_P2P_ROLE_GO) ||
