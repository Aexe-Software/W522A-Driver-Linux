...
{
    if (wifimac && (atomic_read(&wifimac->wm_nrunning) > 0))
    {
        return true;
    }
    else
...
{
    if (wnet_vif && wnet_vif->vm_wmac && (atomic_read(&wnet_vif->vm_wmac->wm_nrunning) == 1)
...
{
    ASSERT(wnet_vif);
    if (wnet_vif->vm_wmac && (atomic_read(&wnet_vif->vm_wmac->wm_nrunning) > 0)
        && (wnet_vif->vm_wmac->wm_runningmask & (~BIT(wnet_vif->wnet_vif_id))))
    {
        return true;
    }
...
#else
static inline void concurrent_channel_protection(struct wlan_net_vif *wnet_vif,int times) {}
static inline void concurrent_vsdb_init(struct wifi_mac *wifimac) {}
static inline unsigned char concurrent_check_is_vmac_same_pri_channel(struct wifi_mac *wifimac) { return 1; }
static inline unsigned char concurrent_check_vmac_is_AP(struct wifi_mac *wifimac) { return 0; }
static inline struct wlan_net_vif *wifi_mac_running_main_wnet_vif(struct wifi_mac *wifimac)
{
    if (wifimac == NULL || wifimac->drv_priv == NULL)
        return NULL;
    return wifimac->drv_priv->drv_wnet_vif_table[NET80211_MAIN_VMAC];
}
static inline struct wlan_net_vif *wifi_mac_running_wnet_vif(struct wifi_mac *wifimac)
{
    return wifi_mac_running_main_wnet_vif(wifimac);
}
